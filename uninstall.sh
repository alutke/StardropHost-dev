...
#
# Non-interactive (dashboard-triggered):
#   UNINSTALL_PORT=18644 STARDROP_REAL_HOME=/home/user sudo bash uninstall.sh
# ===========================================
...
# ===========================================
# Non-interactive mode (dashboard-triggered)
# UNINSTALL_PORT is set by the manager when
# the user clicks Uninstall from the UI.
# ===========================================
if [ -n "$UNINSTALL_PORT" ]; then
    SEARCH_HOME="${STARDROP_REAL_HOME:-$REAL_HOME}"
    TARGET_DIR=""

    for dir in "$SEARCH_HOME/stardrophost" $(for n in $(seq 2 10); do echo "$SEARCH_HOME/stardrophost-$n"; done); do
        [ -f "$dir/.env" ] || continue
        port_val=$(grep "^PANEL_PORT=" "$dir/.env" 2>/dev/null | cut -d= -f2 | tr -d ' \r')
        [ "$port_val" = "$UNINSTALL_PORT" ] || continue
        TARGET_DIR="$dir"
        break
    done

    if [ -z "$TARGET_DIR" ]; then
        echo "[ERR] No instance found with PANEL_PORT=$UNINSTALL_PORT in $SEARCH_HOME"
        exit 1
    fi

    prefix=$(grep "^CONTAINER_PREFIX=" "$TARGET_DIR/.env" 2>/dev/null | tail -1 | cut -d= -f2 | tr -d ' \r')
    [ -z "$prefix" ] && prefix="stardrop"

    echo "[>>] Stopping containers: $prefix"
    if docker compose version &>/dev/null 2>&1; then
        (cd "$TARGET_DIR" && docker compose down --remove-orphans 2>&1) || true
    ...
    rm -rf "$TARGET_DIR"
    echo "[OK] Instance removed."
    exit 0
fi

# ===========================================
# Require root (interactive mode only)
# ===========================================
if [ "$(id -u)" != "0" ]; then
